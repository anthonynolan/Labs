"""
GPT-2 from scratch -- ARIA Circle lab skeleton.

Implement the bodies of the methods marked `raise NotImplementedError(...)`,
in roughly this order:

    Step 1:  TokenEmbedding.__call__
             LearnedPositionalEmbedding.__call__
    Step 2:  scaled_dot_product_attention
             MultiHeadAttention.__call__
    Step 3:  PositionwiseFeedForward.__call__
             DecoderBlock.__call__
    Step 4:  GPT2.forward  (and GPT2.__call__)
    Step 5:  lm_loss
    Step 6:  greedy_generate

Already implemented for you (don't touch unless you want to):
    - GPT2Config (lab-sized defaults)
    - _init_weights (GPT-2's depth-aware init scheme)
    - make_causal_mask
    - CosineScheduleWithWarmup, make_optimizer
    - train_step, train
    - ToyLMDataset (used only by the smoke test in this file)

The smoke test at the bottom runs a tiny config end-to-end. Run this file
(`uv run python skeleton.py`) at any point to see how far you've got --
it'll fail loudly until forward / loss / generate are in place.
